...
pub async fn login(
    ...
    if provider == OAuthProvider::Claude {
        anyhow::bail!(
            "Claudex does not route Claude Free/Pro/Max credentials. Run `claudex models open` and connect Anthropic with a Console API key instead"
        );
    }
...
pub async fn refresh(config: &ClaudexConfig, profile_name: &str) -> Result<()> {
    ...
    match provider {
        OAuthProvider::Claude => anyhow::bail!(
            "Claudex does not refresh or route Claude Free/Pro/Max credentials. Run `claudex models open` and connect an Anthropic Console API key"
        ),

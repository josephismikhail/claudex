...
<Aside type="tip">
  Codex エンドポイントで対応しているモデル: `gpt-5.3-codex`、`gpt-5.1-codex`、`codex-mini-latest`。`gpt-4o` は `chatgpt.com` の Codex エンドポイントでは対応していません。`strip_params = "auto"` により、このエンドポイントを自動検出して `temperature`/`top_p`/`top_k` を除去します。
</Aside>

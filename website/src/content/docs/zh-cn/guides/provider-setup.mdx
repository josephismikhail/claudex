...
<Aside type="tip">
  Codex 端点支持的模型：`gpt-5.3-codex`、`gpt-5.1-codex`、`codex-mini-latest`。注意 `gpt-4o` 在 `chatgpt.com` Codex 端点上不受支持。`strip_params = "auto"` 会自动检测此端点并剥离 `temperature`/`top_p`/`top_k`。
</Aside>

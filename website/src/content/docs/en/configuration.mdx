...
| `[profiles.model_routes]` | `{}` | Exact requested model ID to target profile mapping for mixed-provider sessions |

### Mixed-provider model routes

Use `model_routes` when one Claude Code session should use models backed by different providers. Each key is the exact model ID Claude Code sends; each value names another enabled profile that owns the endpoint and credentials for that model.
...
Define `anthropic` as a separate enabled profile, then run `claudex config validate`. Calls without a matching route continue through `ultracode`.
...
When using OAuth profiles, Claudex points Claude Code at its loopback gateway and sets a local placeholder `ANTHROPIC_AUTH_TOKEN` (not `ANTHROPIC_API_KEY`). The placeholder never leaves the machine; the gateway replaces it with the selected profile's provider credential.

The gateway automatically refreshes OAuth tokens before they expire. You can also manually refresh with:
...
See [`config.example.toml`](https://github.com/josephismikhail/claudex/blob/main/config.example.toml) for a complete configuration file with all providers and options.

...
<Aside type="note">
  Claude OAuth profiles use the loopback gateway, which reads and refreshes Claude Code's local credentials. This keeps cross-provider `model_routes` active. Authenticate once with `claude auth login`; no separate browser flow inside Claudex is required.
</Aside>
...
<Aside type="tip">
  Model availability is determined by the signed-in Codex account. Set `default_model` and the slots to IDs shown by that account. `strip_params = "auto"` detects the ChatGPT Codex endpoint and removes unsupported sampling parameters.
</Aside>

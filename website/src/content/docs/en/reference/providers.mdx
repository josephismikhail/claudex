...
| Claude (Sub) | DirectAnthropic | OAuth | Yes | Yes | Yes | Local gateway + Claude Code credentials |
...
- **Models**: Models available to the signed-in Codex account (for example `gpt-5.6`)
...
- **Models**: `sonnet`, `opus`, `haiku`, or full model IDs allowed by the subscription
- **Note**: Uses the local gateway, which reuses and refreshes Claude Code's OAuth credentials.

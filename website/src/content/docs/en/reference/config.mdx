...
```toml
# Claude Max/Pro (local gateway reuses Claude Code OAuth from ~/.claude)
[[profiles]]
name = "claude-max"
provider_type = "DirectAnthropic"
base_url = "https://api.anthropic.com"
...
<Aside type="caution">
  Every OAuth provider uses the local gateway. Claude credentials come from Claude Code and ChatGPT credentials come from Codex; both can participate in `model_routes` inside one Claude Code session.
</Aside>

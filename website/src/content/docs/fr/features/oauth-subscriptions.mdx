...
Claudex routes Claude OAuth profiles through its local loopback gateway. The gateway reuses Claude Code's credentials from `~/.claude/.credentials.json`, applies the required Bearer OAuth headers, refreshes rotated tokens atomically, and keeps cross-provider `model_routes` active.
...
|-------------|--------------------|--------------------------|--------------------|
| Claude | `https://api.anthropic.com` | DirectAnthropic | `claude-sonnet-4-20250514` |
